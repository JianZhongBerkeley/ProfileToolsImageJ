...
public class ScipyNdimagePercentileFilter extends ScipyFunction {

	protected static final String PROPERTY_SRCPLOT = "srcPlot";
	protected static final String PROPERTY_PERCENTILE = "percentile";
	...
	public ScipyNdimagePercentileFilter() {
		super();
		this.setFuncName("scipy.ndimage.percentile_filter");
		this.properties.addProperty(PROPERTY_SRCPLOT, this.properties.PTYPE_PLOTSTR, "source" , "source", "");
		this.properties.addProperty(PROPERTY_PERCENTILE, this.properties.PTYPE_DOUBLE, "percentile", "percentile", 50.0);
	...
	void processData() {
		...
		// get data from input
		double percentile = (double) this.properties.getPropertyValue(PROPERTY_PERCENTILE);
		...
		try(Interpreter interp = new SharedInterpreter()){
			try {
				double[] dstYs = null;
				double[] dstXs = null;
				interp.exec("from scipy.ndimage import percentile_filter");
				interp.set("percentile", percentile);
				...
				interp.exec("dst = percentile_filter(input=src, percentile=percentile, size=size, mode=mode, cval=cval, origin=origin)");
